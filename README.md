...
DBSCAN isn't be able to achieve an ARI as high as CLASSIX. We have made efforts to choose the best parameters for DBSCAN, though of course one can never be certain they are actually the best possible. In any case, `classix.m` is only a small factor slower than DBSCAN for this dataset, even though DBSCAN is fully implemented in C++.


# CLASSIX on very large datasets


CLASSIX becomes very powerful in particular for extremely large datasets (i.e., many data points) of not too high dimension. Here's an example with more than 2 millions data points for which CLASSIX returns reasonably looking clusterings with a second:
...
```text:Output
  CLASSIX runtime: 0.457 seconds - classes: 7
```



In order to run DBSCAN to completion in reasonable time, we need to downsample to 5% of the data. The hyperparameters are chosen to approximately match the clustering results with CLASSIX.
...
```text:Output
  DBSCAN   runtime:  7.337 seconds - classes: 5
```

# Scaling test: CLASSIX vs DBSCAN


Running DBSCAN on all 2 million data points of the `vdu_signals` dataset is not feasible. Let's see how the runtimes of CLASSIX and DBSCAN scale as the number of data points increases. The plot below shows that, over the range of data sizes of 10k to 100k, CLASSIX's runtime stays in the order of milliseconds, while DBSCAN's runtime grows approximately quadratically until it requires several seconds. The extrapolated DBSCAN runtime on the full dataset with all 2 million points would be about 1 hour. (Timings on MATLAB Online as of October 2023. The precise numbers depend on the machine.)
...
# of data points: 9994
  ...
  DBSCAN  runtime:  0.187 seconds - classes: 5
# of data points: 20087
  CLASSIX runtime:  0.010 seconds - classes: 4
  DBSCAN  runtime:  0.445 seconds - classes: 4
# of data points: 29835
  CLASSIX runtime:  0.011 seconds - classes: 4
  DBSCAN  runtime:  0.833 seconds - classes: 5
# of data points: 39780
  CLASSIX runtime:  0.045 seconds - classes: 5
  DBSCAN  runtime:  1.396 seconds - classes: 6
# of data points: 49483
  CLASSIX runtime:  0.019 seconds - classes: 7
  DBSCAN  runtime:  1.918 seconds - classes: 5
# of data points: 59670
  CLASSIX runtime:  0.023 seconds - classes: 5
  DBSCAN  runtime:  2.695 seconds - classes: 5
# of data points: 69958
  CLASSIX runtime:  0.022 seconds - classes: 6
  DBSCAN  runtime:  3.595 seconds - classes: 7
# of data points: 81152
  CLASSIX runtime:  0.027 seconds - classes: 7
  DBSCAN  runtime:  4.765 seconds - classes: 6
# of data points: 88208
  CLASSIX runtime:  0.029 seconds - classes: 9
  DBSCAN  runtime:  5.716 seconds - classes: 6
# of data points: 101439
  CLASSIX runtime:  0.032 seconds - classes: 8
  DBSCAN  runtime:  7.895 seconds - classes: 5
```
